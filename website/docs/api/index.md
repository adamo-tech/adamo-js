# @adamo-tech/core

## Enumerations

- [HeartbeatState](enumerations/HeartbeatState.md)
- [StreamQuality](enumerations/StreamQuality.md)

## Classes

- [AdamoClient](classes/AdamoClient.md)
- [HeartbeatManager](classes/HeartbeatManager.md)
- [JoypadManager](classes/JoypadManager.md)
- [WebRTCConnection](classes/WebRTCConnection.md)

## Interfaces

- [AdamoClientConfig](interfaces/AdamoClientConfig.md)
- [AdamoClientEvents](interfaces/AdamoClientEvents.md)
- [AdaptiveStreamState](interfaces/AdaptiveStreamState.md)
- [AttachTransformResult](interfaces/AttachTransformResult.md)
- [ControllerState](interfaces/ControllerState.md)
- [ControlMessage](interfaces/ControlMessage.md)
- [CostmapData](interfaces/CostmapData.md)
- [DecodedVideoFrame](interfaces/DecodedVideoFrame.md)
- [EncoderStats](interfaces/EncoderStats.md)
- [HeartbeatConfig](interfaces/HeartbeatConfig.md)
- [HeartbeatMessage](interfaces/HeartbeatMessage.md)
- [JoyMessage](interfaces/JoyMessage.md)
- [JoypadConfig](interfaces/JoypadConfig.md)
- [LatencyBreakdown](interfaces/LatencyBreakdown.md)
- [MapData](interfaces/MapData.md)
- [NavGoal](interfaces/NavGoal.md)
- [NavPath](interfaces/NavPath.md)
- [NetworkStats](interfaces/NetworkStats.md)
- [PingMessage](interfaces/PingMessage.md)
- [PongMessage](interfaces/PongMessage.md)
- [RobotPose](interfaces/RobotPose.md)
- [RobotStats](interfaces/RobotStats.md)
- [SignalingConfig](interfaces/SignalingConfig.md)
- [TopicMessage](interfaces/TopicMessage.md)
- [TrackStreamStats](interfaces/TrackStreamStats.md)
- [VelocityState](interfaces/VelocityState.md)
- [VideoTrack](interfaces/VideoTrack.md)
- [WebCodecsConfig](interfaces/WebCodecsConfig.md)
- [WebCodecsStats](interfaces/WebCodecsStats.md)
- [WebRTCConnectionCallbacks](interfaces/WebRTCConnectionCallbacks.md)
- [WebRTCConnectionConfig](interfaces/WebRTCConnectionConfig.md)

## Type Aliases

- [ConnectionState](type-aliases/ConnectionState.md)
- [WebRTCConnectionState](type-aliases/WebRTCConnectionState.md)

## Functions

- [attachWebCodecsTransform](functions/attachWebCodecsTransform.md)
- [getReceiverForTrack](functions/getReceiverForTrack.md)
- [getVideoReceivers](functions/getVideoReceivers.md)
- [isInsertableStreamsSupported](functions/isInsertableStreamsSupported.md)
- [isRTCRtpScriptTransformSupported](functions/isRTCRtpScriptTransformSupported.md)
- [isWebCodecsSupported](functions/isWebCodecsSupported.md)
